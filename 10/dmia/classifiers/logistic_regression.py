...
class LogisticRegression:
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        prob_class_1 = np.exp(-neg_scores) / (1 + np.exp(-pos_scores - neg_scores))
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        data_loss = -np.mean(y_batch * np.log(prob_class_1 + 1e-15) +
                             (1 - y_batch) * np.log(1 - prob_class_1 + 1e-15))

        # Gradient (averaged)
        errors = prob_class_1 - y_batch
        dw = X_batch.T.dot(errors) / num_train
