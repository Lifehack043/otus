...
```sh
PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python python3 memc_load.py --pattern=*.tsv.gz --dry
```
...
```sh
PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python python3 -m pytest test_memc_load.py -v
```

### Проверка PEP8

```sh
python3 -m pycodestyle memc_load.py test_memc_load.py
```
...
Скрипт использует `concurrent.futures.ThreadPoolExecutor` для параллельной обработки строк внутри каждого файла. Это позволяет эффективно использовать ресурсы CPU и сети при загрузке данных в Memcached.

**Ключевые особенности:**
- Файлы обрабатываются последовательно в хронологическом порядке (сортировка по имени)
- Строки внутри каждого файла обрабатываются параллельно в пуле потоков
- После обработки файла он переименовывается с префиксом `.` для отслеживания прогресса
- Подключения к Memcached кэшируются в пуле `MemcachedClient`
...
| `--workers` | Количество рабочих потоков | `16` |
...
| `--log` | Путь к файлу логов | `None` (stdout) |
| `--test` | Запуск теста protobuf сериализации | `False` |

## Dependencies

```sh
pip install protobuf python-memcached pytest pycodestyle
```
